...
client.on('messageCreate', async (message) => {
 ...
 // --- COMANDO PER GENERARE IMMAGINI ---
    if (message.content.toLowerCase().startsWith('!genera ')) {
        const prompt = message.content.substring(8).trim();
        
        if (!prompt) {
            return message.reply({ content: "❌ Inserisci una descrizione! Esempio: `!genera un PC da gaming con led rossi`" });
        }

        await message.channel.sendTyping();
        
        try {
            // Utilizziamo il nuovo modello unificato "gemini-3.1-flash-image" che è incluso nelle API gratuite
            const response = await genAI.models.generateContent({
                model: 'gemini-3.1-flash-image',
                contents: prompt,
                config: {
                    // Indichiamo all'IA di restituire un'immagine al posto del testo
                    responseModalities: ["IMAGE"]
                }
            });

            // Estraiamo l'immagine codificata in Base64 dalla risposta di Gemini
            ...
            if (!base64Image) {
                return message.reply({ content: "❌ Nessuna immagine generata dal modello." });
            }

            // Convertiamo in buffer leggibile e lo inviamo su Discord
            const buffer = Buffer.from(base64Image, 'base64');
            const attachment = new AttachmentBuilder(buffer, { name: 'gemini-image.png' });

            await message.reply({ content: `🎨 Ecco l'immagine generata per: *"${prompt}"*`, files: [attachment] });
        } catch (error) {
            console.error("[ERRORE GENERAZIONE IMMAGINE]", error?.message || error);
            await message.reply({ content: `❌ Impossibile generare l'immagine.\n**Errore:** \`${error?.message || 'Errore sconosciuto'}\`` });
        }
        return;
    }
