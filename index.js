   ...
   // --- GESTIONE CHAT IA NEI TICKET ---
    if (ticketAIChats.has(message.channel.id)) {
        try {
            ...
            const maxLength = 1950; // Margine di sicurezza rispetto al limite di 2000 di Discord

            // Controllo della lunghezza del messaggio
            if (responseText.length <= maxLength) {
                // Se è sotto il limite, rispondi normalmente
                await message.reply(responseText);
            } else {
                // Se supera il limite, dividi il testo in pezzi
                for (let i = 0; i < responseText.length; i += maxLength) {
                    const chunk = responseText.substring(i, i + maxLength);
                    
                    // Il primo pezzo risponde direttamente all'utente
                    if (i === 0) {
                        await message.reply(chunk);
                    } else {
                        // I pezzi successivi vengono inviati come messaggi normali nel canale
            ...
            await message.reply(`Errore nell'invio del messaggio: ${error?.message || 'errore sconosciuto'}.`);
        }
    }
});
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'btn_wait_admin') {
            ...
            await interaction.followUp({ content: `👤 **Staff Waiting Actived** \nAttendi che uno Staff ti aiuti / Wait for a staff to help you` });
        }
        ...
        if (interaction.customId === 'btn_use_ai') {
            ...
            await interaction.followUp({ content: `✅ **Ai Assistant Actived.**\nSono qui per aiutarti / I'm here to help you` });
        }
