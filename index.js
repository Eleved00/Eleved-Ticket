...
client.on('messageCreate', async (message) => {
   ...
   // --- COMANDO PER GENERARE IMMAGINI ---
    if (message.content.toLowerCase().startsWith('!genera ')) {
        ...
        try {
            // Nota: i parametri aggiuntivi vanno dentro "config"
            const response = await genAI.models.generateImages({
                model: 'imagen-3.0-generate-001', // Usiamo la versione 001 che è la più stabile per le API pubbliche
                prompt: prompt,
                config: {
                    numberOfImages: 1,
                    aspectRatio: '1:1',
            ...
            // Ora il bot risponderà in chat scrivendo ESATTAMENTE qual è il problema di Google
            await message.reply({ content: `❌ Impossibile generare l'immagine.\n**Errore restituito da Google:** \`${error?.message || 'Errore sconosciuto'}\`` });
        }
        return;
    }
