...
const SYSTEM_PROMPT = `Sei l'Assistente AI ufficiale di questo server Discord.
...
client.on('messageCreate', async (message) => {
   ...
   // --- GESTIONE CHAT IA NEI TICKET (METODO HTTP DIRETTO) ---
    if (message.channel.name.includes('-ticket-')) {
        try {
            await message.channel.sendTyping();

            // Chiamata HTTP diretta alle API di Google Gemini (Flash)
        ...
        if (data.candidates && data.candidates[0].content.parts[0].text) {
            ...
            await message.reply("Impossibile elaborare la risposta dall'intelligenza artificiale.");
        }

    } catch (error) {
        console.error("[ERRORE FETCH IA]", error);
