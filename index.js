...
client.on('messageCreate', async (message) => {
    ...
    if (message.content.toLowerCase().startsWith('!genera ')) {
        const prompt = message.content.replace('!genera ', '');
        ...
        try {
            ...
            await message.reply("❌ Si è verificato un errore durante la generazione dell'immagine.");
        }
        return;
    }
    ...
    if (ticketAIChats.has(message.channel.id)) {
        try {
            ...
            await message.reply(`Errore nell'elaborazione: ${error?.message || 'errore sconosciuto'}.`);
        }
    }
});
