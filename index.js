...
server.listen(process.env.PORT || 3000, '0.0.0.0', () => {
    console.log('[SISTEMA] Pagina Web attivata per UptimeRobot');
});
   ...
   // --- GESTIONE CHAT IA NEI TICKET ---
    if (ticketAIChats.has(message.channel.id)) {
        try {
            ...
            // Recupera la sessione di chat esistente per questo canale
            const chat = ticketAIChats.get(message.channel.id);
            
            // Invia il messaggio alla chat attiva di Gemini
            const result = await chat.sendMessage(message.content);
            const responseText = result.response.text();
            
            await message.reply(responseText);
        } catch (error) {
            console.error("[ERRORE DETTAGLIATO GEMINI]:", error);
            await message.reply("Si è verificato un errore nell'elaborazione della risposta AI. Riprova tra poco.");
        }
    }
});
...
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.isButton()) {
        ...
        if (interaction.customId === 'btn_use_ai') {
            ...
            // Crea la sessione di chat isolata
            const chat = model.startChat({
                history: [],
            });
            
            // Salva la sessione associata all'ID del canale
            ticketAIChats.set(interaction.channel.id, chat);

            await interaction.followUp({ content: `✅ **Assistente AI Inizializzato.**\nSono pronto, scrivi pure la tua richiesta.` });
        }
        // Chiusura del ticket
